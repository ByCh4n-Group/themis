    ...
    if [ -e ${1}/CONTROL ] ; then
        ...
        if [ ! -z ${package} ] ; then
            ...
            if [[ $(grep "build() {" "${1}/CONTROL") ]] ; then
                ...
                ascii-art "turtle"
                cd ${1}
                tar -czvf ${setdir}/${package}-${version}.tar.gz ./* && success "the package has been saved: ${Bblue}${setdir}/${package}-${version}.tar.gz${reset}" 
                cd ${setdir}
            else
...
installpackage() {
    [ -d /tmp/themis/cache/pkg ] && rm -rf /tmp/themis/cache/pkg
    mkdir -p /tmp/themis/cache/pkg 
    centexpad "+"
        ...
        if [[ ! -z ${package} ]] ; then
            if [ -d ${pkgmd} ] ; then
                if [[ ! -d ${pkgmd}/${package} ]] ; then
                    ...
                    move() {
                        if [[ ! -z ${1} ]] && [[ ! -z ${2} ]] ; then
                            if [[ -e ${1} ]] ; then
                                ...
                                chown -R ${user}:${group} ${2}/$(basename ${1})
                                [ -e ${2}/${1} ] && chmod +x ${2}/${1}
                                return 0
                            else
                    ...
                    #-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
                    # Bu kısımın gidişi var ama dönüşü yok
